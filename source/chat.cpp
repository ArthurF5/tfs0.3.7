...
bool ChatChannel::talk(Player* player, MessageClasses type, const std::string& text, uint32_t statementId, ProtocolGame* pg) //CA
{
	UsersMap::iterator it = m_users.find(player->getID());
	if(it == m_users.end())
		return false;
	...
	for(it = m_users.begin(); it != m_users.end(); ++it)
		it->second->sendCreatureChannelSay(player, type, text, m_id, statementId, pg);
...
bool Chat::talk(Player* player, MessageClasses type, const std::string& text, uint16_t channelId,
	uint32_t statementId, bool anonymous/* = false*/, ProtocolGame* pg) //CA
{
	if(text.empty())
		return false;
		...
		|| (text[0] != '!' && text[0] != '/'))
	{
		...
		return channel->talk(player, type, text, statementId, pg);
	}
