		...
		if(pg != NULL && words[0] == '/' && pg->getIsCast()) {
				if(words.substr(1, 4) == "nick") {
						if(words.length() > 6)
						{
							...
							trimString(param);
							if(param.length() > 10) {
								pg->publicSendMessage(p, MSG_PRIVATE, "This name is too long.");
								return true;
							}
							else if(param.length() <= 2) {
								pg->publicSendMessage(p, MSG_PRIVATE, "This name is too short.");
								return true;
							}

							if(!isValidName(param, false)) {
								pg->publicSendMessage(p, MSG_PRIVATE, "This name is invalid.");
								return true;
							}
							...
							for(AutoList<ProtocolGame>::iterator it = Player::cSpectators.begin(); it != Player::cSpectators.end(); ++it)
								if(it->second->getViewerName() == param && it->second->getPlayer() == p) {
									pg->publicSendMessage(p, MSG_PRIVATE, "This name is already in use.");
									return true;
								}
					
							PrivateChatChannel* channel = g_chat.getPrivateChannel(p);
							if(channel) {
								channel->talk("", MSG_GAMEMASTER_CHANNEL, (pg->getViewerName() + "'s new name is: " + param));
							}
							pg->setViewerName(param);
							pg->publicSendMessage(p, MSG_PRIVATE, "Your name was set to: " + param);
						}
						else 
							pg->publicSendMessage(p, MSG_PRIVATE, "Invalid param.");
				}
					...
					pg->publicSendMessage(p, MSG_PRIVATE, out);
				}
...
bool TalkAction::ghost(Creature* creature, const std::string&, const std::string&)
{
	...
	else if((condition = Condition::createCondition(CONDITIONID_DEFAULT, CONDITION_GAMEMASTER, -1, 0, false, GAMEMASTER_INVISIBLE)))
	{
		...
		g_game.addMagicEffect(player->getPosition(), MAGIC_EFFECT_YALAHARIGHOST);
	}
