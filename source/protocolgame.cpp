	...
	OperatingSystem_t operatingSystem, uint16_t version, bool gamemaster, bool castAccount) //CA
{
	...
	if((!_player || g_config.getNumber(ConfigManager::ALLOW_CLONES) || (accountManager && name == "Account Manager")) && !castAccount)
	{
		isCast = false; //CA
	...
	addRef();
	return connect(_player->getID(), operatingSystem, version, castAccount); //CA
}
	...
	if(getIsCast()) { //CA
		...
		for(AutoList<ProtocolGame>::iterator it = Player::cSpectators.begin(); it != Player::cSpectators.end(); ++it) //CA
			if(it->second == this) {
					...
					if(channel) 
						channel->talk("", MSG_GAMEMASTER_CHANNEL, (getViewerName() + " has left the cast."));
...
bool ProtocolGame::connect(uint32_t playerId, OperatingSystem_t operatingSystem, uint16_t version, bool castAccount) //CA
{
	if(!castAccount)
		unRef();
	...
	if(castAccount) {  //CA
		...
		if(_player->getCastViewerCount() >= 10) {
			disconnectClient(0x14, "The cast reached the maximum viewer limit (10).");
			return false;
		}
		...
		if(channel) {
			...
			channel->talk("", MSG_GAMEMASTER_CHANNEL, (getViewerName() + " has joined the cast."));
			publicSendMessage(player, MSG_PRIVATE, "Cast communication is turned on.");
		} else 
			publicSendMessage(player, MSG_PRIVATE, "Cast communication is turned off.");
		return true;
	}
...
void ProtocolGame::onRecvFirstMessage(NetworkMessage& msg)
{
	...
	bool castAccount = false; //CA
	if(name.empty())
	{
        if(!g_config.getBool(ConfigManager::ENABLE_CAST)) { //CA
    		if(!g_config.getBool(ConfigManager::ACCOUNT_MANAGER))
    		{
	...
	Dispatcher::getInstance().addTask(createTask(boost::bind(
		&ProtocolGame::login, this, character, id, password, operatingSystem, version, gamemaster, castAccount))); //CA
}
...
{
	if(isCast && !(creature->getPlayer() == player) && type != MSG_SPEAK_SAY) //CA
		return;
...
void ProtocolGame::sendCreatureChannelSay(const Creature* creature, MessageClasses type, const std::string& text, uint16_t channelId, uint32_t statementId, ProtocolGame* pg) //CA
{
    ...
    TRACK_MESSAGE(msg);
    AddCreatureSpeak(msg, creature, type, text, channelId, NULL, statementId, pg);
}
...
void ProtocolGame::AddCreatureSpeak(NetworkMessage_ptr msg, const Creature* creature, MessageClasses type,
	std::string text, uint16_t channelId, Position* pos, uint32_t statementId, ProtocolGame* pg) //CA
{
	msg->put<char>(0xAA);
	if(creature)
	{
		...
		std::string pname;
		if(creature->getPlayer() && pg && pg->getIsCast()) //CA 
			pname = pg->viewerName;
		else 
		...
		if(speaker && !speaker->isAccountManager() && !speaker->hasCustomFlag(PlayerCustomFlag_HideLevel) && (pg == NULL || (pg != NULL && !pg->getIsCast()))) //CA
