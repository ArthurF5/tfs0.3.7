...
bool Game::removeCreature(Creature* creature, bool isLogout /*= true*/)
{
	...
	if(Player* p = creature->getPlayer()) { //CA
		...
		for(AutoList<ProtocolGame>::iterator it = Player::cSpectators.begin(); it != Player::cSpectators.end(); ++it) {
			if(it->second->getPlayer() == p) {
				Player::cSpectators.erase(it->first);
				it = Player::cSpectators.begin();
			}
		}
...
bool Game::playerCreatePrivateChannel(uint32_t playerId, ProtocolGame* pg) //CA
{
	...
	if(pg != NULL && pg->getIsCast()) {
		...
		if(channel) {
			...
			pg->publicSendMessage(player, MSG_PRIVATE, "The cast owner disabled communication.");
	...
	for(AutoList<ProtocolGame>::const_iterator it = Player::cSpectators.begin(); it != Player::cSpectators.end(); ++it) if(it->second->getPlayer() == player)
		it->second->publicSendMessage(player, MSG_PRIVATE, "The cast owner turned communication on.");
...
bool Game::playerRequestChannels(uint32_t playerId, ProtocolGame* pg) //CA
{
	...
	if(pg != NULL && pg->getIsCast()) {//CA
		ChannelsList list;
		list.push_back(std::make_pair(CHANNEL_PRIVATE, "Cast Channel"));
		pg->publicSendChannelsDialog(list);
    }
	else {
...
bool Game::playerCloseChannel(uint32_t playerId, uint16_t channelId)
{
	...
	if(channel && channel->getId() == channelId) {
		for(AutoList<ProtocolGame>::const_iterator it = Player::cSpectators.begin(); it != Player::cSpectators.end(); ++it) if(it->second->getPlayer() == player)
			it->second->publicSendMessage(player, MSG_PRIVATE, "The cast owner turned communication off.");
	}
...
bool Game::playerSay(uint32_t playerId, uint16_t channelId, MessageClasses type, const std::string& receiver, const std::string& text, ProtocolGame* pg) //CA
{
	Player* player = getPlayerByID(playerId);
	if(!player || player->isRemoved())
		return false;
	...
	if (pg != NULL && pg->getIsCast()) {  //CA
		if(g_talkActions->onPlayerSay(player, type == MSG_SPEAK_SAY ? (unsigned)CHANNEL_DEFAULT : channelId, text, false, pg))
			return true;

		PlayerCast pc = player->getCast();
		for(std::list<CastBan>::iterator it = pc.muted.begin(); it != pc.muted.end(); ++it)
			if(it->ip == pg->getIP()) {
				pg->publicSendMessage(player, MSG_PRIVATE, "You are muted on this cast.");
				return false;
			}

		uint32_t statementId = 0;
		IOLoginData::getInstance()->playerStatement(player, channelId, text, statementId);
		if(playerSpeakToChannel(player, type, text, channelId, statementId, pg))
			return true;
		else
			return false;
	}
...
bool Game::playerSpeakToChannel(Player* player, MessageClasses type, const std::string& text, uint16_t channelId, uint32_t statementId, ProtocolGame* pg) //CA
{
	switch(type)
	{
		case MSG_CHANNEL:
	...
	if(text.length() < 251)
		return g_chat.talk(player, type, text, channelId, statementId, false, pg);
