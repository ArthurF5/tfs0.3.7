...
void ProtocolLogin::onRecvFirstMessage(NetworkMessage& msg)
{
	...
	Account account;
	if((!IOLoginData::getInstance()->loadAccount(account, name) || !encryptTest(password, account.password)) && !castAccount) //CA
	{
	...
	if(OutputMessage_ptr output = OutputMessagePool::getInstance()->getOutputMessage(this, false))
	{
		...
		if(castAccount)
	  	...
	  	else if(g_config.getBool(ConfigManager::ACCOUNT_MANAGER) && account.number != 1)
		{
