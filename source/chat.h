...
class ChatChannel
{
	public:
		...
		bool talk(Player* player, MessageClasses type, const std::string& text, uint32_t statementId, ProtocolGame* pg = NULL); //CA
...
class Chat
{
	public:
		...
		bool talk(Player* player, MessageClasses type, const std::string& text,
			uint16_t channelId, uint32_t statementId, bool anonymous = false, ProtocolGame* pg = NULL); //CA
