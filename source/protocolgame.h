...
class ProtocolGame : public Protocol
{
	public:
		...
		void publicSendChannelsDialog(ChannelsList& list) {
			sendChannelsDialog(list);
		}
		void publicSendCreatePrivateChannel(uint16_t channelId, const std::string& channelName) { 
			sendCreatePrivateChannel(channelId, channelName);
		}
		void publicSendMessage(const Creature* creature, MessageClasses type, const std::string& text) {
			uint32_t statementId = 0;
			sendCreatureSay(creature, type, text, NULL, statementId);
		}
	...
	private:
		...
		void sendCreatureChannelSay(const Creature* creature, MessageClasses type, const std::string& text, uint16_t channelId, uint32_t statementId, ProtocolGame* pg); //CA
		...
		void AddCreatureSpeak(NetworkMessage_ptr msg, const Creature* creature, MessageClasses type,
			std::string text, uint16_t channelId, Position* pos, uint32_t statementId, ProtocolGame* pg = NULL); //CA
