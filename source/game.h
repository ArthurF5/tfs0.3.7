...
class Game
{
	public:
		...
		bool playerSay(uint32_t playerId, uint16_t channelId, MessageClasses type,
			const std::string& receiver, const std::string& text, ProtocolGame* pg); //CA
	...
	protected:
		...
		bool playerSpeakToChannel(Player* player, MessageClasses type, const std::string& text, uint16_t channelId, uint32_t statementId, ProtocolGame* pg = NULL); //CA
